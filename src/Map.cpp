...
namespace tutorial
{
          ...
          height_(height)
    {
        ...
        if (!console_) {
            throw std::runtime_error("Failed to create map console");
        }
    ...
    void Map::ComputeFov(pos_t origin, int fovRadius)
    {
        ...
        TCOD_map_compute_fov(map_, origin.x, origin.y, fovRadius, true, FOV_RESTRICTIVE);
    }
    ...
    void Map::SetTileType(pos_t pos, TileType type)
    {
        ...
        switch (type)
        {
            case TileType::FLOOR:
                ...
                TCOD_map_set_properties(map_, pos.x, pos.y, true, true);
                break;
            case TileType::WALL:
                // transparent = false, walkable = false
                TCOD_map_set_properties(map_, pos.x, pos.y, false, false);
                break;
            default:
                break;
        }
    ...
    void Map::Update()
    {
        ...
        for (std::size_t i = 0; i < tiles_.size(); ++i)
        {
            ...
            TCOD_console_set_char_background(console_, pos.x, pos.y, color, TCOD_BKGND_SET);
        }
    }
    ...
    void Map::Render(TCOD_Console* parent) const
    {
        ...
        TCOD_console_blit(console_, 0, 0, width_, height_, parent, 0, 0, 1.0f, 1.0f);
    }

    void Map::Clear()
    {
        rooms_.clear();
        TCOD_map_clear(map_, false, false);  // Set all tiles to non-transparent, non-walkable
