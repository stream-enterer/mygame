...
namespace tutorial
{
    ...
    void Map::Render(TCOD_Console* parent) const
    {
        ...
        TCOD_console_blit(console_, 0, 0, width_, height_, parent, 0, 0, 1.0f,
                          1.0f);
    }
    ...
    void Map::UpdateScent(pos_t playerPos)
    {
        ...
        for (int x = 0; x < width_; ++x)
        {
            for (int y = 0; y < height_; ++y)
            {
                ...
                if (IsInFov(pos))
                {
                    auto& tile = tiles_.at(util::posToIndex(pos, width_));
                    ...
                    // New scent = current turn value minus distance
                    unsigned int newScent = currentScentValue_ - distance;

                    // Only update if new scent is stronger than old
