...
namespace tutorial
{
	...
	EffectData EffectData::FromJson(const json& j)
	{
		...
		if (!j.contains("type")) {
			throw std::runtime_error(
			    "Effect missing required 'type' field");
		}
		data.type = j["type"];
	...
	TargetingData TargetingData::FromJson(const json& j)
	{
		...
		if (!j.contains("type")) {
			throw std::runtime_error(
			    "Targeting missing required 'type' field");
		}
		data.type = j["type"];
	...
	ItemData ItemData::FromJson(const json& j)
	{
		...
		if (!j.contains("targeting")) {
			throw std::runtime_error(
			    "Item missing required 'targeting' field");
		}
		...
		for (const auto& effectJson : j["effects"]) {
			data.effects.push_back(
			    EffectData::FromJson(effectJson));
		}
	...
	json EntityTemplate::ToJson() const
	{
		...
		if (item.has_value()) {
			...
			if (item->targeting.range.has_value()) {
				targetingJson["range"] =
				    item->targeting.range.value();
			}
			if (item->targeting.radius.has_value()) {
				targetingJson["radius"] =
				    item->targeting.radius.value();
			}
			...
			for (const auto& effect : item->effects) {
				...
				if (effect.messageKey.has_value()) {
					effectJson["messageKey"] =
					    effect.messageKey.value();
				}
