...
namespace tutorial
{
    ...
    void StringTable::LoadLocale(const std::string& locale)
    {
        ...
        try
        {
            ...
            std::cout << "[StringTable] Loaded locale: " << locale << std::endl;
        }
    ...
    std::string StringTable::GetString(const std::string& key) const
    {
        ...
        if (!value)
        {
            std::cerr << "[StringTable] WARNING: Missing string key '" << key
                      << "'" << std::endl;
            return "[MISSING: " + key + "]";
        }
        ...
        const std::unordered_map<std::string, std::string>& params) const
    {
        ...
        if (!value)
        {
            std::cerr << "[StringTable] WARNING: Missing message key '" << key
                      << "'" << std::endl;
        ...
        if (value->is_string())
        {
            std::string text = FormatString(value->get<std::string>(), params);
            return LocalizedMessage(text, tcod::ColorRGB{ 255, 255, 255 },
                                    false);
        }
        ...
        const std::unordered_map<std::string, std::string>& params) const
    {
        ...
        {
            text = FormatString(msgJson["text"].get<std::string>(), params);
        }
        ...
        {
            color = tcod::ColorRGB{
                static_cast<uint8_t>(msgJson["color"][0].get<int>()),
                static_cast<uint8_t>(msgJson["color"][1].get<int>()),
                static_cast<uint8_t>(msgJson["color"][2].get<int>())
            };
        }
