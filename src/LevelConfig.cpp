...
namespace tutorial
{
    ...
    LevelConfig LevelConfig::FromJson(const nlohmann::json& j)
    {
        ...
        config.generation.minRoomSize = params.value("min_room_size", 6);
        config.generation.maxRoomSize = params.value("max_room_size", 10);

        // Parse monster spawning
        if (j.contains("spawning") && j["spawning"].contains("monsters"))
        {
            ...
            {
                for (const auto& entry : monsters["spawn_table"])
                {
                    std::string id = entry["id"];
                    int weight = entry["weight"];
                    config.monsterSpawning.spawnTable.push_back({ id, weight });
                }
            }
        }
        ...
        if (j.contains("spawning") && j["spawning"].contains("items"))
        {
            ...
            config.itemSpawning.maxPerRoom = items.value("max_per_room", 2);
            config.itemSpawning.spawnChance = items.value("spawn_chance", 0.7f);
                ...
                && items["spawn_table"].is_array())
            {
                for (const auto& entry : items["spawn_table"])
                {
                    std::string id = entry["id"];
                    int weight = entry["weight"];
                    config.itemSpawning.spawnTable.push_back({ id, weight });
                }
            }
        }
        ...
        std::cout << "  - Monster spawn entries: "
                  << config.monsterSpawning.spawnTable.size() << std::endl;
